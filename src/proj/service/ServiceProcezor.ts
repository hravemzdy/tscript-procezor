...
export abstract class ServiceProcezor implements IServiceProcezor {
    ...
    InitWithPeriod(period: IPeriod): boolean {
        const log: Logger = new Logger({ name: "ProcezorServiceLogger" });
        ...
        if (initResult === false) {
            log.error(`Period: ${period.code}, init with period failed`)
        }
        return initResult;
    }
    ...
    BuildFactories(): boolean {
        ...
        if (!(articleFactorySuccess && conceptFactorySuccess)) {
            log.error(`Version: ${this.version}, build factories failed`);
        }
